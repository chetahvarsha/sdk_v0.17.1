...
pub trait DcdtNftMarketplace: storage::StorageModule + views::ViewsModule {
	#[init]
	...
	fn auction_token(
		...
		require!(
			accepted_payment_token.is_rewa() || accepted_payment_token.is_valid_dcdt_identifier(),
	...
	fn withdraw(&self, auction_id: u64) -> SCResult<()> {
		...
		self.transfer_dcdt(&caller, nft_type, nft_nonce, &nft_amount, b"returned token");
	...
	fn transfer_dcdt(
		...
		// nonce 0 means fungible DCDT or REWA
		if nonce == 0 {
			self.send()
