...
pub trait Lottery {
	...
	fn start_lottery(
		...
		match opt_burn_percentage {
			OptionalArg::Some(burn_percentage) => {
				require!(!token_name.is_rewa(), "REWA can't be burned!");

				let roles = self.blockchain().get_dcdt_local_roles(&token_name);
				require!(
	...
	fn distribute_prizes(&self, lottery_name: &BoxedBytes) {
		...
		if burn_percentage > 0 {
			...
			let roles = self.blockchain().get_dcdt_local_roles(&info.token_name);
			if roles.contains(&DcdtLocalRole::Burn) {
				self.send().dcdt_local_burn(&info.token_name, &burn_amount);
			}
