...
pub fn generate_proxy_sig(method: &Method) -> proc_macro2::TokenStream {
	...
	let result = quote! {
		fn #method_name #generics (
			self,
			#(#arg_decl),*
		) -> numbat_wasm::types::ContractCall<Self::SendApi, <#ret_tok as numbat_wasm::io::EndpointResult>::DecodeAs>
		#generics_where
	};
	result
}
				...
				let sig = quote! {
					#msig {
						...
						let mut ___contract_call___ = numbat_wasm::types::new_contract_call(
							...
							#payment_expr,
							#nonce_expr,
							numbat_wasm::types::BoxedBytes::from(#endpoint_name_literal),
						);
						#(#arg_push_snippets)*
						___contract_call___
					}
