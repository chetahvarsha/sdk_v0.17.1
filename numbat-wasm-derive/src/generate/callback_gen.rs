...
pub fn generate_callback_selector_and_main(
	...
	if let Some(raw) = raw_decl {
		...
		let cb_main_body = quote! {
			let _ = self.callback_selector(numbat_wasm::hex_call_data::HexCallDataDeserializer::new(&[]));
		};
		...
		if match_arms.is_empty() && module_calls.is_empty() {
			let cb_selector_body = quote! {
				numbat_wasm::types::CallbackSelectorResult::NotProcessed(___cb_data_deserializer___)
			};
			let cb_main_body = quote! {};
			(cb_selector_body, cb_main_body)
		} else {
			...
			let cb_main_body = quote! {
				let ___tx_hash___ = numbat_wasm::api::BlockchainApi::get_tx_hash(&self.blockchain());
				let ___cb_data_raw___ = numbat_wasm::api::StorageReadApi::storage_load_boxed_bytes(&self.get_storage_raw(), &___tx_hash___.as_bytes());
				numbat_wasm::api::StorageWriteApi::storage_store_slice_u8(&self.get_storage_raw(), &___tx_hash___.as_bytes(), &[]); // cleanup
				let mut ___cb_data_deserializer___ = numbat_wasm::hex_call_data::HexCallDataDeserializer::new(___cb_data_raw___.as_slice());
		...
		.map(|supertrait| {
			...
			quote! {
				match #module_path EndpointWrappers::callback_selector(self, ___cb_data_deserializer___) {
					...
					numbat_wasm::types::CallbackSelectorResult::NotProcessed(recovered_deser) => {
