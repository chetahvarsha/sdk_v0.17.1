...
pub fn contract_implementation(
	...
	let endpoint_wrappers = quote! {
		pub trait EndpointWrappers:
			numbat_wasm::api::ContractPrivateApi
			+ #trait_name_ident
		...
		#where_self_big_int
		{
			...
			fn callback_selector<'a>(&self, mut ___cb_data_deserializer___: numbat_wasm::hex_call_data::HexCallDataDeserializer<'a>) -> numbat_wasm::types::CallbackSelectorResult<'a> {
	...
	let wasm_endpoints = quote! {
		...
		mod endpoints {
			...
			fn new_andes_instance() -> super::ContractObj<numbat_wasm_node::AndesApiImpl> {
