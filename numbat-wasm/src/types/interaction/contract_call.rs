...
use crate::{
	api::{BigUintApi, SendApi, DCDT_NFT_TRANSFER_STRING, DCDT_TRANSFER_STRING},
	...
	SA: SendApi + 'static,
{
	...
	fn convert_to_dcdt_transfer_call(self) -> Self {
		if self.payment_token.is_rewa() {
			self
		} else if self.payment_nonce == 0 {
			// fungible DCDT
			let mut new_arg_buffer = ArgBuffer::new();
			new_arg_buffer.push_argument_bytes(self.payment_token.as_dcdt_identifier());
			...
			new_arg_buffer.push_argument_bytes(
				numbat_codec::top_encode_no_err(&self.payment_nonce).as_slice(),
			);
			new_arg_buffer.push_argument_bytes(self.payment_amount.to_bytes_be().as_slice());
	...
	pub fn async_call(mut self) -> AsyncCall<SA> {
		...
		AsyncCall {
			api: self.api,
			to: self.to,
			rewa_payment: self.payment_amount,
